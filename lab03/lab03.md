...
#### Решение
$\frac{56 \text{ байт}}{128 \text{ Кбит/с}} + \frac{56 \text{ байт}}{1 \text{ Мбит/с}} + 5 \text{мс} =  3.5 \text{мс} + 0.448 \text{мс} + 5 \text{мс} = 8.948 \text{мс}$. 
Если же имеется ввиду, что время декодирование на хосте Б тоже учитвается, то еще $+\frac{56 \text{ байт}}{128 \text{ Кбит/с}} = 8.948 + 3.5 = 12.448$
...
#### Решение
$N = 10 + 1$ пакетов (так как плюс пакет, который передается в данный момент).
Средняя задержка передачи равна $10$мс, тк $100$ пакетов за $1000$мс.
$d = 10 + 10 = 20$мс.
$a = \frac{11}{20} = 0.55$ пакетов/мс.
...
#### Решение
1. Второй пакет отправится, когда первый дойдет до середины, а время в пути у них одиннаковое. При этом ждать в середине второму пакету не придется, так как узким местом является первая линия. То есть разница равна $\frac{L}{R_S}$ - время, за которое пакет (первый) добирается до середины.

2. Второй пакет может находится во входном буфере, ожидая передачи на вторую линию, если он отправится с сервера через время $T < \frac{L}{R_C} - \frac{L}{R_S}$ поле отпавки первого. В таком случае, когда второй пакет попадет в середину, пройдет $T + \frac{L}{R_S}$ времени с момента полной отправки первого, а это меньше $\frac{L}{R_C}$. Значит первый пакет, еще не дошел до клиента, и второму надо подождать в буфере. Ну а соответственно, если $T \geq \frac{L}{R_C} - \frac{L}{R_S}$, очередь будет нулевая.
...
#### Решение
1. $\Delta = \frac{850000 \text{ бит}}{15Мбит/c} = \frac{17}{300}$с. (так как 15Мбит/c - узкое место последовательного соединения каналов со скорстями 15 и 100)
2. $3 + \frac{17/300}{1 - 17/300 \cdot 16}  = 3\frac{17}{28} \text{ с}$

3. $0.4 \cdot (3 + \frac{17/300}{1 - 16 \cdot 0.4 \cdot 17/300}) + 0.6 \cdot \frac{850000 \text{ бит}}{100Мбит/c} \approx 1.236 \text{ с}+ 0.085\text{ с} \approx 1.32 \text{ с}$